...
词汇按常用度参考排序。每个 JLPT 等级按页面渲染顺序最前的 **640 个词**均配有独立人工审核例句，共 **3,200 个连续覆盖词汇例句**；此外仍保留部分精选词的人工例句。当前全站共有 **3,277 条**审核词汇例句，没有审核例句的词条不会显示例句。
...
`vocab-examples-review.js` 保存早期精选词的人工复查例句；`vocab-examples-highfreq-*` 分批为各级词汇提供独立、人工审核的高质量例句，目前已按页面渲染顺序覆盖每级前 640 词。频率排序本身不会产生任何例句。
...
- 每级按页面渲染顺序前 640 词全部具有人工审核例句（共 3,200 条，CI 使用可独立调整的分级阈值）
